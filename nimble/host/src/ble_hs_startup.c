...
static int ble_hs_startup_read_sup_f_tx(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS, BLE_HCI_OCF_IP_RD_LOC_SUPP_FEAT), NULL,
                         0, &rsp, sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_read_local_ver_tx(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS, BLE_HCI_OCF_IP_RD_LOCAL_VER), NULL, 0,
                         &rsp, sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_read_sup_cmd_tx(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS, BLE_HCI_OCF_IP_RD_LOC_SUPP_CMD), NULL,
                         0, &rsp, sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_le_read_sup_f_tx(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RD_LOC_SUPP_FEAT), NULL, 0, &rsp,
                         sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_le_read_buf_sz_tx(uint16_t *out_pktlen, uint8_t *out_max_pkts) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_RD_BUF_SIZE), NULL, 0, &rsp,
                         sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_read_bd_addr(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_INFO_PARAMS, BLE_HCI_OCF_IP_RD_BD_ADDR), NULL, 0,
                         &rsp, sizeof(rsp));
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_le_set_evmask_tx(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_LE, BLE_HCI_OCF_LE_SET_EVENT_MASK), &cmd,
                         sizeof(cmd), NULL, 0);
  if (rc != 0) {
    return rc;
  }
...
static int ble_hs_startup_set_evmask_tx(void) {
  ...
  rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND, BLE_HCI_OCF_CB_SET_EVENT_MASK), &cmd,
                         sizeof(cmd), NULL, 0);
  if (rc != 0) {
    return rc;
  }
  ...
  if ((version >= BLE_HCI_VER_BCS_4_1) && ((sup_cmd.commands[22] & 0x04) != 0)) {
    ...
    rc = ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND, BLE_HCI_OCF_CB_SET_EVENT_MASK2),
...
static int ble_hs_startup_reset_tx(void) {
  return ble_hs_hci_cmd_tx(BLE_HCI_OP(BLE_HCI_OGF_CTLR_BASEBAND, BLE_HCI_OCF_CB_RESET), NULL, 0,
                           NULL, 0);
}
...
int ble_hs_startup_go(void) {
  ...
  if (ble_hs_cfg.store_gen_key_cb) {
    ...
    rc = ble_hs_cfg.store_gen_key_cb(BLE_STORE_GEN_KEY_IRK, &gen_key, BLE_HS_CONN_HANDLE_NONE);
    if (rc == 0) {
